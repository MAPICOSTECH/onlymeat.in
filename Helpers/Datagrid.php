...
class Datagrid {
    ...
    public $source; //source for this array
    protected $colWidths; //array of widths for each cols
    protected $colMapping; //array of columns whose content needs to be hyperlinked.
    protected $stripSlashes; //strip slashes in cell data displayed
 ...
 $this->tableCssClass = 'table table-hover';
    ...
    public function render() {
        ...
        for ($i = 0; $i < $this->source->colCount; $i++) {
            ...
            if ($this->preserveColumnNames == false) {
                $colName = ucwords(str_replace(array('_'), ' ', $this->source->fields[$i]));
            } else
    ...
    public function setRowAlternativeColors($color1, $color2) {
        $this->_alternativeColors = array($color1, $color2);
    }
     ...
     * 
     * @param string $pageToLink Denote with a caret e.g./Mypage/page/^/id/23
     * @param string $displayText Denote page number with caret e.g. Page - ^
     * @param int $totalRecCount 
     * @param int $perPageCount
     * @param int $highlightPage page number to be highlighted
     * @param bool $useDefaultLayout if true plain text, if false, colored boxes
     */
